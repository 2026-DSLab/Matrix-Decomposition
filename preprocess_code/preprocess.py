...
def filter_by_min_interactions(ratings: pd.DataFrame, min_interactions: int) -> pd.DataFrame:
    """상호작용(평점) 개수가 min_interactions 이상인 유저의 평점만 남김."""
    counts = ratings.groupby("userId").size()
    qualified_users = counts[counts >= min_interactions].index
...
def main(min_interactions: int, n_sample_users: int, seed: int, raw_dir: str, out_dir: str):
    os.makedirs(out_dir, exist_ok=True)

    ratings = load_ratings(raw_dir)
    filtered = filter_by_min_interactions(ratings, min_interactions)

    csv_path = os.path.join(out_dir, f"ratings_min{min_interactions}.csv")
    ...
    xlsx_path = os.path.join(out_dir, f"ratings_min{min_interactions}_{n_sample_users}users.xlsx")
...
if __name__ == "__main__":
    ...
    main(args.min_interactions, args.n_sample_users, args.seed, args.raw_dir, args.out_dir)